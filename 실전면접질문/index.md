...
<!-- 220118 진도 -->
## 17. CSS에서 'C’는 Cascading을 의미합니다. Cascading에 관해서 설명해주세요. 또 cascading system의 장점은 무엇인가요?
Cascading이란 말 그대로 폭포처럼 위에서부터 아래로 흐르는다는 뜻 입니다. 상위에 설정되어있는 상속관계대로 아래에도 스타일이 흐르듯이 적용이 되고, 이런 특징을 이용해 보다 간결하고 효율적으로 코드를 작성 가능합니다. +) 하지만 레이아웃에 영향을 주는 속성들은 상속되지 않아 해당하지 않습니다.

## 18. 프로토타입이란 무엇인가요?

프로토타입이란 자바스크립트에서 객체를 상속하기 위해 사용하는 방식입니다. 모든 객체는 원형이 되는 프로토타입 객체를 가지며 이를 공통으로 참조합니다. 프로토타입 객체의 메소드와 속성은 복사되는 것이 아니라, 프로토타입을 타고 올라가 접근이 가능합니다. 이를 이용하여 코드를 재사용 할 수 있습니다. 

- 프로토타입이란 자바스크립트에서 객체를 상속하기 위해 사용하는 방식입니다. 다른 언어에서 보통 사용되는 '클래스'가 자바스크립트에는 존재 하지 않기 때문에 -> ㅇㅣ젠 있다..! 고안된 방식이며,
- 기존의 객체를 복사하여 새로운 프로토타입을 생성하여 사용합니다. -> 프로토타입 체이닝에 대한 내용

> 모든 객체는 공통으로 참조하는 공간, 즉 원형이 되는 프로토타입 객체를 가집니다. 

## 19. DTD란 무엇인가요?? 만약 없다면 어떤일이 일어나나요?
DTD란 Document Type Definition으로 문서 형식 정의를 뜻합니다. 보통 문서의 상단에 선언하게 되며 흔히 html에서 `<!DOCTYPE html>`의 형식으로 사용합니다.
...
- quirks(쿽스)모드가 나온 이유
  - 예전엔 브라우저 별로 페이지를 생성했고 너무 비효율적이라 웹 표준이 생김. 브라우저는 표준 이전의 문서와 이후의 문서를 렌더링해야하기 때문에 생긴 것이 쿽스모드
  - DTD가 선언되어있으면 표준모드, 없다면 쿽스모드

## 20. 서버 사이드 랜더링과 클라이언트 사이드 랜더링의 차이
서버 사이드 렌더링(SSR)은 페이지 이동시마다 서버에 새로운 페이지를 요청하여 로딩하는 것이고, 클라이언트 사이드 렌더링(CSR)은 최초 1회만 전체 페이지를 로딩하고 이후에는 클라이언트 액션에 따라 필요한 부분만 렌더링하는 것 입니다.
서버 사이드 렌더링은 이동하게 되는 페이지 각각의 정보를 받아오기 때문에 비교적 초기 로딩속도가 빠르고 보다 정확한 검색 최적화가 가능합니다. 클라이언트 사이드 렌더링은 최초 1회에 부분 렌더링을 고려한 모든 데이터를 로딩해야해서 비교적 초기 로딩속도가 느리지만 별도의 리로딩 없이 사용자 액션에 따라 빠른 페이지 제공이 가능합니다.

- 렌더링은 브라우저가 한다.
- '다만 매번 각 페이지를 생성하고 설정해야하므로  복잡도가 높습니다.' -> 내가 설명이 힘든 부분은 말하지 말자.. 아님 확실히 알아두자..

> 서버사이드 렌더링은 서버에서 데이터를 불러와 HTML을 다 만든 후 한번에 내려주고, 클라이언트 사이드 렌더링은 클라이언트에서 데이트를 불러와 HTML을 클라이언트에서 생성합니다.  빈 HTML 파일로 시작하여 JS 파일을 불러오면 JS가 DOM을 생성해 화면에 뿌려주는 방식입니다. CSR은 페이지 이동 중 나오는 짧은 흰색화면이 나오지 않습니다. 
